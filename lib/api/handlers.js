...
function posts(request, reply) {
  runner
    ...
    .run(respond(reply, state => pick(state, 'posts', 'totalCount', 'pageCount')));
}
...
function unsubscribe(request, reply) {
  runner
    .init(runnerOptions(request))
    .deleteSubscription({ subscriptionId: request.params._id, soft: true })
    .run(respond(reply, state => state.subscription));
}
...
function savePostQuery(request, reply) {
  runner
    .init(runnerOptions(request))
    .maybeFetchPostQuery(request.payload)
...
function fetchTags(request, reply) {
  runner
    .init(runnerOptions(request))
    .fetchTags()
    .run(respond(reply, state => state.data));
}

function saveTags(request, reply) {
  runner
    .init(runnerOptions(request))
    .saveTags(request.payload)
    .fetchTags()
