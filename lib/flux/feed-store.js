...
class FeedStore extends EventEmitter {
  ...
  findFeed(feedId) {
    return this.state.feeds.findIndex(fd => fd._id === feedId);
  }
  ...
  addFeed(data) {
    const index = this.findFeed(data.feed._id);
  ...
  updateReadCount(post) {
    const feedId = post.feedId,
          amount = post.read ? -1 : +1;

    trans(this.state.feeds)
      .filter('_id', id => id === feedId)
  ...
  setFeeds(data) {
    ...
    const feeds = trans(data.feeds)
            .mapff('_id', 'subscription', subscriptions)
            .value();
...
dispatcher.register(action => {
  ...
  case constants.feeds.SUBSCRIBE_DONE:
    store.updateSubscription(action.feed._id, action.subscription);
    break;
  case constants.feeds.UNSUBSCRIBE_DONE:
    store.updateSubscription(action.feed._id);
    break;
